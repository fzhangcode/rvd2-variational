...
def test(caseHDF5Name, controlHDF5Name, alpha=0.05, tau=0, chi2=False, outputFile=None):
    ...
    for j in xrange(J):
        ...
        controlMu.append(beta_mean(controlgam[j,:])-controlphi['mu0'])
        caseMu.append(beta_mean(casegam[j,:])-casephi['mu0'])
    ...
    if outputFile is not None:
        ...
        write_dualvcf(vcfFilename, loc, call, refb, controlMu,\
        controlR, controlN, caseMu, caseR, caseN)
        # output hdf5 file
    ...
    ## output the results
def write_dualvcf(outputFile, loc, call, refb, controlMu=None, controlR=None, controlN=None, caseMu=None, caseR=None, caseN=None):
    ...
    for i in xrange(J):
        ...
        if call[i]:           
            ...
            print ("chr%s\t%d\t.\t%s\t.\t.\tPASS\tCOAF=%0.3f;CAAF=%0.3f\tAU:CU:GU:TU\t%d:%d:%d:%d\t%d:%d:%d:%d" \
                   % (chrom[i], pos[i], refb[i], controlMu[i]*100, caseMu[i]*100,\
                      controlR4[0], controlR4[1], controlR4[2], controlR4[3],\
                      caseR4[0], caseR4[1], caseR4[2], caseR4[3]), file=vcfF) 
    vcfF.close()
