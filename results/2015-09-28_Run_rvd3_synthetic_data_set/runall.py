...
tocfilename = "./synthetic_toc_p1_test.txt"
...
try:
    ...
    controlFileList = ["./depth_chart/10/%s" % filename for filename in toc.Filename[toc.isRef=='Y']]
...
for dilution in np.unique(toc[toc.isRef=='N'].Dilution):
    ...
    try:
        ...
        caseFileList = ["./depth_chart/10/%s" % filename for filename in toc.Filename[toc.Dilution==dilution]]
