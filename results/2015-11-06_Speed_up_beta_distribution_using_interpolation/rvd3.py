...
from scipy.interpolate import interp1d, splint, UnivariateSpline, splrep
...
'''# Use cubic splin interpolation for Beta function. There is an error.
def EqlogGamma(gam, M): 
    mu = np.arange(1e-3, 1-1e-3, 0.01)

    #f = interp1d(mu, kernel(mu, gam, M), kind='cubic')
    #munew = np.arange(1e-3, 1-1e-3, 0.01)
    #temp = f(munew)  
    #print (temp)  
    #logGamma = temp[-1]
    
    spl = UnivariateSpline(mu, kernel(mu, gam, M))
    logGamma = spl.integral(1e-3, 1-1e-3)

    print (logGamma)
    return logGamma'''
    

# Use cubic splin interpolation for Beta function
# The initial ELBO is very close to the one based on integration.
def EqlogGamma(gam, M): 
    mu = np.arange(1e-6, 1-1e-6, 0.0001)
...
# Calculate the integral of the cubic splin
def integ(x, tck, constant=-0.1):
     x = np.atleast_1d(x)
...
def opt_par(func, x, args, bnds, parlabel):
    ...
    '''if res.success == False:
        logging.debug(3)
        res = so.minimize(func, x, args=args, bounds=bnds, method='Nelder-Mead') #, options={'xtol': 1e-8, 'disp': True} ) '''

    '''if res.success == False:
        logging.debug(4)
        res = so.minimize(func, x, args=args, bounds=bnds, method='Newton-CG' ) '''
        
    '''if res.success == False:
        logging.debug(5)
        res = so.minimize(func, x, args=args, bounds=bnds, method='CG' ) '''
        
    # use the gradient of the objective function, which can be given by the user. 
    if res.success == False:
        logging.debug(6)  
        res = so.minimize(func, x, bounds=bnds,\
            args=args, method='BFGS', options={'disp': True}) # quasi-Newton method of Broyden, Fletcher, Goldfarb, and Shanno 
...
def ELBO_opt(r, n, phi = None, q = None, seed = None, pool = None, vaf = None):
    ...
    h5file = "./tmp.hdf5"
    logging.info('Storing model updates in %s' % h5file)  
