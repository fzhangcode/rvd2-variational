...
]{\inits{PJ}\fnm{Patrick J} \snm{Flaherty}}
...
\end{equation}
subject to the constraints that $\delta_{ji1} \geq 0$ and $\delta_{ji2} \geq 0$ and conditioned on fixed values for the other model and variational parameters using Sequential Least SQuares Programming (SLSQP) \cite{kraft1988software}.
...
\end{equation}
subject to the constraints that $\gamma_{j1} \geq 0$ and $\gamma_{j2} \geq 0$ and conditioned on fixed values for the other model and variational parameters using SLSQP \cite{kraft1988software}.
...
\begin{split}
  ...
  &\mathcal{N}(E_q[\mu_j|r^{case}] - E_q[\mu_j|r^{control}], \text{Var}_q[\mu_j|r^{case}] + \text{Var}_q[\mu_j|r^{control}])
\end{split}
\end{equation}
